...
if [[ -z $FOLDERNAME ]]; then
    ...
    echo "${0} -f <FOLDER> -i <ITEMNAME> -s <SPOTIFYID> -c <CARDID>"
    exit 1
fi
...
# Card
if [[ ! -z $CARD ]]; then
    echo "Create card ${CARD}"
    sudo echo $FOLDERNAME/$ITEMNAME > $AUDIOFOLDERSPATH/../shortcuts/$CARD
    sudo chown -R :www-data $AUDIOFOLDERSPATH/../shortcuts/$CARD
    sudo chmod -R 777 $AUDIOFOLDERSPATH/../shortcuts/$CARD
fi